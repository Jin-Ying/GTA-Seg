...
from .base import BaseUnlabeledDataset


class coco_dset(BaseUnlabeledDataset):
    def __init__(
        self, data_root, data_list, trs_form, seed=0, n_sup=5000
    ):
    ...
    def __getitem__(self, index):
        ...
        label_path = os.path.join(self.data_root, self.list_sample_new[index][0])
        image = self.img_loader(image_path, "RGB")
        label = self.img_loader(image_path, "L")
...
def build_cocoloader(all_cfg, seed=0):
    cfg_dset = all_cfg["dataset_coco"]

    cfg = copy.deepcopy(cfg_dset)

    workers = cfg.get("workers", 2)
    batch_size = cfg.get("batch_size", 1)
    n_sup = cfg.get("n_sup", 5000)
    # build transform
